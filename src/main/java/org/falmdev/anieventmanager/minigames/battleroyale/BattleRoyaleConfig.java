...
/**
 * Configuración del Battle Royale.
 * plugins/AniEventManager/minigames/battleroyale.yml
 *
 * La zona se calcula automáticamente desde la arena (pos1/pos2):
 *   - Centro: punto medio de pos1/pos2
 *   - Radio máximo: mitad del lado más corto de la arena
 *   - 5 fases con porcentajes: 100% → 60% → 30% → 10% → 2%
 *
 * En el yaml solo se configura: tiempos (wait/shrink) y daño por fase.
 */
public class BattleRoyaleConfig {

    /** Porcentajes del radio máximo para las 5 fases. */
    public static final double[] ZONE_PHASE_PERCENTAGES = { 1.00, 0.60, 0.30, 0.10, 0.02 };

    /** Radio mínimo absoluto (en bloques) — la última zona nunca es menor que esto. */
    ...
    private void writeDefaults() {
        ...
        // Fases — solo tiempos y daño. El radio se calcula automáticamente.
    ...
    /** Datos de una fase de zona — el radio se calcula, solo wait/shrink/damage en yaml. */
    public record ZonePhaseData(double radius, int waitSeconds, int shrinkSeconds, double damagePerSecond) {}

    /**
     * Calcula el centro de la zona automáticamente como el punto medio
     * de pos1/pos2 de la arena.
     */
    ...
    /**
     * Calcula el radio máximo de la zona = mitad del lado más corto de la arena.
     * Garantiza que la primera zona quepa dentro del rectángulo.
     */
    ...
    /**
     * Devuelve las fases con radio calculado automáticamente desde la arena.
     * Los porcentajes están en ZONE_PHASE_PERCENTAGES.
     */
    public List<ZonePhaseData> getZonePhases() {
        ...
        if (maxRadius <= 0) return list; // arena no configurada
        ...
        for (int i = 0; i < phaseCount; i++) {
            ...
            // Radio calculado automáticamente desde el porcentaje
